...
const returnAnimationTime = (time) => {
  ...
  const returnAnimationDisplayTime = (time) => {
    // 7시(420분)부터 시작하므로 +7 시간 추가
    const totalMinutes = 420 + Math.round(time);
    const hour = addZeroFill(parseInt((totalMinutes / 60) % 24));
    const minute = addZeroFill(totalMinutes % 60);
    return [hour, minute];
  };
...
const Trip = (props) => {
  ...
  // const trip = currData(props.trip, time);  // 현재 시간에 해당하는 trip만 표시 
  ...
  const layers = [
    new TripsLayer({  
      ...
      getColor: [255, 255, 0],  // 노란색 (Chapter 4 스타일)
      opacity: 1,
      widthMinPixels: 7,
      rounded: true,
      capRounded : true,
